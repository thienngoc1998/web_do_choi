...
class AdminProductController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->export) {
            return \Excel::download(new ProductsExport($products->get()), 'san-pham'. Carbon::now() .'.xlsx');
        }
    ...
    {
        try {
            if ($request->numberImport && $request->ID) {
                $id = $request->ID;
                $product           = Product::find($id);
                $data['pro_number']     = $request->numberImport + $product->pro_number;
                $update = $product->update($data);
                if ($update) {
                    return response('Nhập hàng thành công', 200)
                        ->header('Content-Type', 'text/plain');
                } else {
                    return response('Nhập hàng thất bại vui lòng thử lại', 200)
                        ->header('Content-Type', 'text/plain');
                }
